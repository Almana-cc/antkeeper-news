#!/bin/bash
# Ralph - Autonomous AI Agent Loop for Amp CLI
# Runs Amp repeatedly until all PRD tasks are complete
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Ralph - Amp Agent Loop${NC}"
echo -e "${BLUE}========================================${NC}"
...
if ! command -v amp &> /dev/null; then
    echo -e "${RED}Error: amp CLI is required but not installed${NC}"
...
"
    ...
    # Run Amp with execute mode and auto-approve all tools
    echo -e "${BLUE}Running Amp...${NC}"
    OUTPUT=$(amp --dangerously-allow-all -x < "$TEMP_PROMPT" 2>&1) || true
    rm -f "$TEMP_PROMPT"
